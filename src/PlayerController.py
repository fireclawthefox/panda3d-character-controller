...
class PlayerController(FSM, Config, Physics, Actor, Control, Animator):
    ...
    def __init__(self, physic_world, config=None):
        ...
        if self.use_simple_shadow:
            ...
            self.shadow = OnscreenImage(self.simple_shadow_image)
            self.shadow.setP(-90)
    ...
    def changeCameraSystem(self, mode, initMode=False):
        ...
        if mode == "firstperson":
            ...
            self.camera_handler = CameraFirstPerson(
                ...
                self.cam_fov)
            self.loadAnims({
                self.IDLE: self.anim_idle_fp,
                self.WALK: self.anim_walk_fp,
                self.RUN: self.anim_run_fp,
                self.SPRINT: self.anim_sprint_fp,
                self.JUMP_START: self.anim_jumpstart_fp,
                self.JUMP_LAND: self.anim_jumpland_fp,
                self.FALL: self.anim_falling_fp,
                self.CROUCH: self.anim_crouch_move_fp,
                self.CROUCH_IDLE: self.anim_crouch_idle_fp,
                self.CRAWL: self.anim_crawl_move_fp,
                self.CRAWL_IDLE: self.anim_crawl_idle_fp,
                self.ROLL: self.anim_roll_fp,})
            ...
            self.loadAnims({
                ...
                self.RUN: self.anim_run,
                self.SPRINT: self.anim_sprint,
                self.JUMP_START: self.anim_jumpstart,
                self.JUMP_LAND: self.anim_jumpland,
                self.FALL: self.anim_falling,
                self.CROUCH: self.anim_crouch_move,
                self.CROUCH_IDLE: self.anim_crouch_idle,
                self.CRAWL: self.anim_crawl_move,
                self.CRAWL_IDLE: self.anim_crawl_idle,
                self.ROLL: self.anim_roll,})
        else:
