...
class Animator:
    ...
    def cleanup(self):
        if self.pauseFromIdleTask is not None:
            taskMgr.remove(self.pauseFromIdleTask)
    ...
    def setCurrentAnimsPlayRate(self, rate):
        ...
        for anim in self.current_animations:
            ...
            base.messenger.send(self.audio_set_walk_playrate_evt, [rate])
    ...
    #
    def enterIdle(self):
        base.messenger.send(self.audio_stop_walk_evt)
        self.current_animations = [self.IDLE]
        if not self.getCurrentAnim() == self.IDLE:
            self.loop(self.IDLE)

        self.pauseFromIdleTask = taskMgr.doMethodLater(
            self.idle_to_pause_time,
            base.messenger.send,
            self.idle_to_pause_task_name,
            extraArgs=[self.idle_to_pause_event_name])
    ...
    def enterIdleToWalk(self):
        base.messenger.send(self.audio_play_walk_evt)
        self.current_animations = [self.IDLE, self.WALK]
        self.ease_out_idle.duration = self.enterWalkDuration
        self.ease_in_walk.duration = self.enterWalkDuration
    ...
    def enterIdleToRun(self):
        base.messenger.send(self.audio_play_run_evt)
        self.current_animations = [self.IDLE, self.RUN]
        self.ease_out_idle.duration = self.enterRunDuration
        self.ease_in_run.duration = self.enterRunDuration
    ...
    def enterIdleToSprint(self):
        base.messenger.send(self.audio_play_sprint_evt)
        self.current_animations = [self.IDLE, self.SPRINT]
        self.ease_out_idle.duration = self.enterSprintDuration
        self.ease_in_sprint.duration = self.enterSprintDuration
    ...
    def enterWalkToRun(self):
        ...
        self.ease_in_run.duration = self.enterRunDuration
        self.ease_out_walk.duration = self.enterRunDuration
    ...
    def enterRunToWalk(self):
        ...
        self.ease_in_walk.duration = self.enterWalkDuration
        self.ease_out_run.duration = self.enterWalkDuration
    ...
    def enterRunToSprint(self):
        ...
        self.ease_in_sprint.duration = self.enterSprintDuration
        self.ease_out_run.duration = self.enterSprintDuration
    ...
    def enterSprintToRun(self):
        ...
        self.current_animations = [self.WALK, self.RUN]
        self.ease_in_run.duration = self.enterRunDuration
        self.ease_out_sprint.duration = self.enterRunDuration
    ...
    def enterJump(self):
        base.messenger.send(self.audio_stop_walk_evt)
        base.messenger.send(self.audio_play_jump_evt)
    ...
    def enterFall(self):
        base.messenger.send(self.audio_stop_walk_evt)
        base.messenger.send(self.audio_play_fall_evt)
        if self.getCurrentAnim() != self.FALL and self.getCurrentAnim() != self.JUMP_START:
            self.loop(self.FALL)

    def enterLand(self):
        base.messenger.send(self.audio_play_land_evt)
