...
class Physics:
    ...
    def __init__(self):
        ...
        self.player_capsule = BulletCapsuleShape(self.player_radius, self.player_height/2.0)
        self.baseZOff = self.player_height/2.0
    ...
    def updatePlayerPos(self, speed, heading, dt, rotation=None, rotate_around_node=None, ignore_step=False):
        ...
        if heading is not None:
            ...
            self.mainNode.setP(0)
            self.mainNode.setR(0)
        self.mainNode.setPos(self.mainNode, speed)
        self.ignore_step = ignore_step
        if not ignore_step:
            if self.doStep(self.state == self.STATE_IDLE):
                self.mainNode.node().setAngularVelocity((0, 0, 0))
                self.mainNode.node().setLinearVelocity((0, 0, 0))
    ...
    def doStep(self, preventSlipping=False):
        ...
        if self.state not in self.ignore_step_states:
            ...
            if char_step_collision is not None:
                if self.hasSurfacePoint(char_step_collision):
                    ...
                    self.mainNode.setZ(pos.getZ() + self.baseZOff)
                    return True
            return False
        ...
        #    self.toggleFlyMode(False)
            ...
            if ledge_node is not None:
                if ledge_node.getName().startswith(self.platform_collision_prefix):
    ...
    def updateCharSimpleShadow(self):
        ...
        shadow/ground"""
        self.raylist[self.shadow_ray_id].point_a = self.mainNode.getPos()
    ...
    def processJumping(self, dt):
        ...
        self.player_capsule_np.setZ(
            ...
            self.jump_strength * self.jump_time)
