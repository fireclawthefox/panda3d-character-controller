...
class Control:
    ...
    but glues together most of the other classes and is responsible for
    ...
    def move(self, task):
        ...
        self.rotation = None
        self.isMoving = self.current_accleration > 0
        self.isAirborn = False
        self.update_speed = Point3()
        self.plugin_setMoveDirection(Vec3(0, 0, 0))
        ...
        and self.state not in self.flying_states:
            ...
            and self.cur_jump_press_time > self.max_jump_press_time:
        ...
        if self.wasJumping:
            ...
            self.do_jump = self.do_jump or self.cur_jump_press_time <= self.min_jump_press_time
        ...
        if self.isAirborn:
            ...
            if self.do_sprint and self.can_use_sprint:
                # sprint
                accleration = self.accleration_sprint
                self.current_max_accleration = self.max_accleration_sprint
            elif self.do_walk:
                # walk
                # set accleration
                accleration = self.accleration_walk
                self.current_max_accleration = self.max_accleration_walk
            else:
                # run
                accleration = self.accleration_run
        ...
        if self.active_platform is not None:
            platformPositionAbsolute = self.active_platform.getPos()
            ...
            if self.respect_platform_rotation:
                if self.last_platform_rotation is None:
                    self.last_platform_rotation = self.active_platform.getH()
                platform_rotation = self.active_platform.getH() - self.last_platform_rotation
                self.last_platform_rotation = self.active_platform.getH()
        ...
        #
        if self.do_jump \
        and self.jump_enabled \
        and self.fall_time <= self.jump_allow_after_fall_time: #\
            if self.STATE_JUMP in self.defaultTransitions[self.state]:
                ...
                self.cur_jump_press_time += self.dt
            if self.cur_jump_press_time <= self.max_jump_press_time:
                forward_speed = self.speed * self.current_accleration
                ...
                if self.platform_movement_affects_jump:
        ...
        #
        if self.isAirborn and self.state not in self.flying_states:
            ...
            if self.move_key_pressed:
                ...
                self.current_speed = max(
                    self.speed_airborn * self.dt,
                    self.speed * self.current_accleration * self.dt)
                self.update_speed = Point3(0, -self.current_speed, 0)
            else:
                # No movement key was pressed, so move the player according
                # to previous speed
                self.current_speed = self.speed * self.current_accleration * self.dt
                self.update_speed = Point3(0, -self.current_speed, 0)
        ...
        use_stamina = use_stamina and not self.isAirborn
        ...
        use_stamina = use_stamina and self.isMoving
        ...
        if use_stamina:
            self.stamina -= self.stamina_usage_per_second * self.dt
            if self.stamina <= 0:
                self.stamina_was_empty = True
        elif self.stamina < self.max_stamina:
            regain_stamina = self.dt * self.stamina_recover_per_second_run
            if self.state in self.walk_states:
                regain_stamina = self.dt * self.stamina_recover_per_second_walk
            elif self.state is self.STATE_IDLE or self.STATE_FALL:
                regain_stamina = self.dt * self.stamina_recover_per_second_idle
        ...
        if self.active_platform is not None and not self.state in self.jump_and_fall_states:
            ...
            if self.respect_platform_rotation:
                self.updatePlayerHprFloating(platform_rotation, self.active_platform)
        else:
            self.last_platform_position = None
            self.last_platform_rotation = None
